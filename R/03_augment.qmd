...
In order to be able to distinguish patients participating in the trial a Unique Subject Identifier `USUJBID` is added a variable number one in the cleaned dataset.
...
```{r}
data_split_1 <- data_clean_groups |> 
  select(1:50)

data_split_2 <- data_clean_groups |> 
  select(1, 51:98)
```
