...
p1 <- data |> 
  ...
  #Illustrating the mean value
  geom_segment(x = mean_age,
               xend = mean_age, 
               y = 0, 
               yend = 2900, 
               color = "darkgreen", 
               linetype = "dashed", 
               size = 1.0) +
  #Annotating the mean age with a text
  annotate("label", 
           x = mean_age + 12,                     
           y = 2400,                     
...
p2 <- dataset |> 
  ggplot(mapping = aes(x = STYPE,
                       y = AGE,
                       fill = STYPE)) +
  geom_boxplot(alpha = 0.7,
               outlier.shape = NA) + 
  # Boxplot without outlier points   
  labs(title = "Stroke subtypes distributed by age",
       x = "Stroke Type (STYPE)",
       y = "Age (years)") +   
  theme_minimal(base_size = 14) + 
  theme(plot.title = element_text(hjust = 0.5,
...
# Saving the plot as a PNG file
ggsave(here("results/03_description_plot_2.png"), 
       plot = p2, 
       width = 8,
       height = 4,
       dpi = 300,
...
# Saving the plot as a PNG file
ggsave(here("results/03_description_plot_3.png"), 
       plot = p1, 
       width = 8, 
       height = 4, 
       dpi = 300, 
