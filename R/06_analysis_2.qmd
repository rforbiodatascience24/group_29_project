...
ggplot(dataset, aes(x = AGE)) +
  ...
  scale_y_continuous(expand = expansion(mult = c(0, 0.05)),breaks = seq(0, 3000, by = 500),) + # Avoids crowded y-axis and setting breaks
# Adding a short dashed line for the mean age
    ...
    linetype = "dashed",
    size = 1.0
  ) + # Adds mean age as a vertical dashed line
  annotate(
    "label",
    ...
    label = paste("Mean Age:", round(mean(data$AGE, na.rm = TRUE), 1),"years"),
    color = "white",
    fill="darkgreen",
    fontface="bold",
    size=5,
...
ggplot(dataset, aes(x = STYPE, fill = SEX)) +
  geom_bar() +
  scale_fill_manual(values = c("blue", "pink")) +
  labs(
    title = "Gender Distribution of the Study Population",
    x = "Treatment groups",
    y = "Number of People"
  ) +
  ...
  theme(
    ...
    legend.position = "none" # Hide legend since labels are on the x-axis
  )
