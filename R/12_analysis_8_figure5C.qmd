...
## Data analysis - XXX FIND OUT WHAT TO REPLACE H14 with

Creation of figure 5C
...
In order to make the correct plot, it is necessary to use the same variables as the authors of the article. In order to validate that the correct variables have been identified a table summing the number of patients in each group is made: \## Creating Function

```{r}
data |> 
  group_by(RXASP,RXHEP,H14) |> 
  count()
```

The numbers here seems to fit the numbers from figure 5.C to the extent that it can be read. XXX
...
```{r}
relevant_data <- data |> 
  select(RXHEP, RXASP, H14) 
```

The plot has percentages of reported XXX on the y-axis. Such a variable is created.
